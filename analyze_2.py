...
CACHE_FILE = "./family_analysis_out/soap_cache.pkl"

# =====================================================================
# CONFIG  -- edit this section
# =====================================================================

# Path to data.txt (the file you showed me)
DATA_TXT = "data.txt"

PRUNE_STALE_CACHE = True
UMAP_RANDOM_STATE = 42

# Directory containing gen_<number>.cif files
CIF_DIR = "./../cifs"
...
REFERENCES = {
    ...
    # add more as needed, e.g. "gamma_ordering_2": "./refs/foo.cif"
}
...
def parse_data_txt(path):
    """Parse data.txt -> DataFrame(mattergen_id, gen_id, e_hull). Skips N/A gen_ids."""
    rows = []
    with open(path) as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith("-") or line.startswith("Results"):
                continue
            if "Mattergen ID" in line:
                continue
            parts = [p.strip() for p in line.split("|")]
            if len(parts) != 3:
                continue
            mg_id, gen_id, ehull = parts
            if gen_id == "N/A":
                continue
            try:
                ehull = float(ehull)
                ...
                continue
            rows.append({"mattergen_id": mg_id, "gen_id": gen_id, "e_hull": ehull})
    return pd.DataFrame(rows)


def load_structures(df, cif_dir):
    """Returns list of (mattergen_id, gen_id, atoms, e_hull, abs_path)."""
    items = []
    for _, row in df.iterrows():
        path = Path(cif_dir) / row["gen_id"]
        if not path.exists():
            print(f"[warn] missing CIF: {path}")
            continue
        atoms = read(str(path))
        items.append((row["mattergen_id"], row["gen_id"], atoms, row["e_hull"], str(path)))
    return items
...
def main():
    ...
    # 1. Parse data.txt
    print("[1/7] Parsing data.txt ...")
    df = parse_data_txt(DATA_TXT)
    ...
    all_atoms = [a for _, _, a, _, _ in gen_items] + [a for _, _, a in ref_items]
    if MERGE_SI_AL:
    ...
    gen_paths_atoms = [(path, atoms) for _, _, atoms, _, path in gen_items]
    ...
    n_ref = len(ref_items)
    out_rows = []
    for i, (mg, gid, _, eh, _) in enumerate(gen_items):
        row = {
            "mattergen_id": mg,
            "gen_id": gid,
            "e_hull": eh,
    ...
    for c in sorted(set(gen_clusters)):
        mask = gen_clusters == c
        size = int(mask.sum())
        mean_eh = float(np.mean([e for i, (_, _, _, e, _) in enumerate(gen_items) if mask[i]]))
    ...
    e_hulls = np.array([eh for _, _, _, eh, _ in gen_items])
